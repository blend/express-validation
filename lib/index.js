    ...
    var validate = function(current, request, schema, location, allowUnknown){
      ...
      Joi.validate(request, schema, {context: request, allowUnknown : allowUnknown, abortEarly : false}, function(errors, value){
        ...
        _.each(errors.details, function(error){
          ...
          if (!errorExists) {
            // In Joi 11.0+ path changes from period-delimited-string to array. It is no longer set by default to error.context.label (previously error.context.key)
            // https://github.com/hapijs/joi/issues/1302
            current.push({ field : error.path.join('.') || error.context.label, location : location, messages : [error.message], types:[error.type]});
          }
